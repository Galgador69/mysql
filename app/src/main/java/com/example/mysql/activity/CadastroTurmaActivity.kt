...
class CadastroTurmaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (turmaId != -1) {
            binding.edtNomeTur.setText(intent.getStringExtra("TURMA_NOME"))
        }

        binding.btnSaveTur.setOnClickListener {
            val nome = binding.edtNomeTur.text.toString()
        ...
        call.enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@CadastroTurmaActivity, "Erro ao salvar aluno.", Toast.LENGTH_SHORT).show()
                }
            }
